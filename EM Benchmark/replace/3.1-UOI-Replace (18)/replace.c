...
int	maxset;
{
	...
	if (src[*i - 1] == ESCAPE) {
	    escjunk = esc(src, i);
	    junk = addstr(escjunk, dest, j, maxset--);   //mutated statement
	} else	
	    if (src[*i] != DASH)
  ...
  char *	sub;
{
	...
	if ((sub[i] == DITTO))
	    for (j = s1; j < s2; j++) 
	    {
		fprintf(fout, "%c",lin[j]);
	    }	
	else	
	{
	    fprintf(fout,"%c",sub[i]);
	}
	i = i + 1;
    }
}
...
main(int argc, char	*argv[3], char *line)
{
